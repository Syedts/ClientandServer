...
int i ;
	 ...
	 for(i = 0 ; i < 10; i++){
	if(pthread_create(&thread1[i],NULL, receiveFunc,(void *)sockfd))perror("failed to create thread");
	}
	//if(pthread_create(&threadList[1],NULL, sendFunc,(void *)sockfd))perror("failed to create thread");
while(true) 
{
